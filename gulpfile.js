/**
 *
 * citygate stacked animation gulpfile.js
 *
 */
...
// base
config.siteName = 'citygate';
config.serverDir = './build/';
config.proxyDomain = process.env.BROWSERSYNC_PROXY_DOMAIN || 'citygate.test';
...
gulp.task('default', callback =>
  runSequence(
    ['sass', 'scripts', 'svg', 'img', 'pug-build'],
    'browsersync',
    'watch',
    callback
  )
...
gulp.task('production', callback =>
  runSequence(
    ['set-prod-node-env', 'sass', 'scripts', 'svg', 'img', 'pug-build'],
    callback
  )
);
